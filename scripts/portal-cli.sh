...
case "$1" in
  ...
  pe|port-edit)
    echo -e "${CYAN}📊 正在打开端口账本文件 (server/data/port_ledger.json)...${NC}"
    ${EDITOR:-nano} "${PORTAL_DIR}/server/data/port_ledger.json"
    ;;
  ...
  ${YELLOW}dnp pe${NC}    (p-edit)    - 快速打开端口账本 (port_ledger.json) 进行编辑或手动指定老端口"
